...
def test_retrieve_node_returns_documents():
    ...
    mock_retriever.retrieve.assert_called_once_with("테스트 질문", top_k=20, filter_doc_ids=[])
...
def test_retrieve_node_uses_question_field():
    ...
    mock_retriever.retrieve.assert_called_once_with("특정 질문", top_k=20, filter_doc_ids=[])
...
def test_retrieve_node_uses_rewritten_question_when_available():
    ...
    mock_retriever.retrieve.assert_called_once_with("재작성 질문", top_k=20, filter_doc_ids=[])
...
def test_retrieve_node_falls_back_to_question_when_rewritten_empty():
    ...
    mock_retriever.retrieve.assert_called_once_with("원본 질문", top_k=20, filter_doc_ids=[])
...
def test_retrieve_node_passes_allowed_doc_ids():
    ...
    mock_retriever.retrieve.assert_called_once_with(
        "질문", top_k=20, filter_doc_ids=["docs/company/policy.md"]
    )
...
def test_retrieve_node_passes_empty_filter_when_no_acl():
    ...
    mock_retriever.retrieve.assert_called_once_with("질문", top_k=20, filter_doc_ids=[])
