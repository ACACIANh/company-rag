...
# ADR-0053: hallucination 재시도 시 RAG_GENERATE 뒤에 덧붙이는 grounding 교정 지시.
# 직전 답변이 문서에 근거하지 못했으므로, 동일 입력 재호출 대신 "문서 근거 강제 + 없으면 모른다"로
# 입력을 바꿔 실패 모드(문서 이탈/추측)를 깬다. 문서가 답을 안 담은 경우엔 정직한 비답변을 유도한다.
REGENERATE_GROUNDING_NOTICE = """

[재시도 지침] 직전 답변이 위 문서에 충분히 근거하지 않았습니다. 이번에는 반드시 위 참고 문서의 내용만으로 답하고, 문서에서 확인할 수 없는 내용은 추측하지 말고 "제공된 문서에서 확인할 수 없습니다"라고 답하세요."""
