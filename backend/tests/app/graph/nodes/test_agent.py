...
def test_agent_seeds_human_message_on_first_turn():
    ...
    out = agent_node({"agent_messages": [], "question": "전직원 급여 보여줘", "rewritten_question": "전직원 급여 조회 방법은?"}, chat_model=model)
...
def test_agent_seeds_original_question_not_rewritten():
    ...
    state = {"question": "alice를 engineering 부서에 추가해줘",
             ...
             "agent_messages": []}
    agent_node(state, chat_model=_Model())
