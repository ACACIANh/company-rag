...
권한 부여가 **부서 멤버십 부여와 강제로 결합**돼 있다. "민준에게 인사 문서 하나를 보여주고 싶다"가 시스템상 "민준을 인사팀에 통째로 넣기"가 되고, 그 결과 인사팀의 **모든 폴더 + 테이블 + SQL 권한 + 조직상 의미 + 부서 admin 위임 대상**이 함께 딸려온다. 최소권한 원칙 위반.
