...
def test_department_membership_tuple():
    tuples = _build_tuples([{"user_id": "user-joohwan", "departments": ["개발"]}], {}, {})
    assert _find(tuples, user="user:user-joohwan", relation="member", object="department:개발")
...
def test_non_admin_does_not_get_capability_admin_grant():
    tuples = _build_tuples([{"user_id": "user-joohwan", "roles": ["user"]}], {}, {})
    assert not _find(tuples, user="user:user-joohwan", relation="justify_grant", object="capability:admin")


# ── 부서 관리자 위임(ADR-0046) ──────────────────────────────
def test_dept_admin_of_emits_admin_tuple():
    tuples = _build_tuples([{"user_id": "user-joohwan", "dept_admin_of": ["개발"]}], {}, {})
    assert _find(tuples, user="user:user-joohwan", relation="admin", object="department:개발")
...
async def test_prune_deletes_only_stale():
    ...
    live = [
        ("user:user-joohwan", "member", "department:개발"),          # 유지(desired)
    ...
    # 유지 대상은 절대 삭제하지 않는다
    for u, r, o in [("user:user-joohwan", "member", "department:개발")]:
