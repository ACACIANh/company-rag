...
### 1. 먼저 생각하고 코딩하라
가정을 명시하고, 혼란을 숨기지 말고, 트레이드오프를 드러낼 것.

- 구현 전 가정을 명시적으로 말할 것. 불확실하면 질문하라.
- 해석이 여러 가지라면 조용히 고르지 말고 제시하라.
- 더 단순한 방법이 있다면 말할 것. 필요하면 반론을 제기하라.
- 무엇이 불명확한지 이름 붙이고 질문하라.

### 2. 단순함을 우선하라
요청한 것만 구현. 추측성 기능·추상화·에러 핸들링 금지.

- 요청 범위를 넘는 기능 추가 금지.
- 단일 사용 코드에 추상화 금지. (단, `shared/`의 ABC·구현체는 교체 가능성·학습 목적으로 유지)
- 요청하지 않은 유연성·설정 가능성 추가 금지.
- 발생 불가능한 시나리오의 에러 핸들링 금지.
- 200줄로 쓴 코드가 50줄로 가능하다면 다시 쓰라.

### 3. 수술적으로 변경하라
요청에 해당하는 줄만 수정. 내가 만든 쓰레기만 치운다.

- 인접 코드·주석·포맷을 "개선"하지 말 것.
- 고장나지 않은 코드를 리팩터링하지 말 것.
- 기존 스타일(따옴표, 타입힌트 유무, 공백)을 그대로 유지.
- 관련 없는 dead code를 발견하면 언급만 하고 삭제하지 않는다.
- 내 변경이 만들어낸 unused import·변수·함수는 직접 정리한다.
- 검증: 변경된 모든 줄은 사용자 요청과 직접 연결되어야 한다.

### 4. 목표 중심으로 실행하라
작업을 검증 가능한 목표로 전환. 계획 → 검증 루프.

- "추가해줘" → "이 테스트가 통과하면 완료"로 변환.
- "고쳐줘" → "버그를 재현하는 테스트를 먼저 작성, 그 다음 통과"로 변환.
- 멀티스텝 작업은 착수 전에 명시:
  ```
  1. [단계] → 검증: [확인 방법]
  2. [단계] → 검증: [확인 방법]
